...
//Cabecera de página
   ...
   function Header()
   {
       if ($this->PageNo() == 1){
        ...
        //Salto de línea
            ...
            $this->Cell(60,5,utf8_decode($tipo));
            $this->Ln(8);
            $this->SetFont('Arial','',12);
            $this->Cell(-1);
            $this->Cell(60,5,utf8_decode('Técnico:  '.$tecnico));
            $this->Ln(5);
            $this->Cell(-1);
            $this->Cell(60,5,utf8_decode('Fecha Inicio:  '.strftime("%A, %d de %B del %Y", strtotime($fechaem)).'.-'));
            $this->Ln(5);
            $this->Cell(-1);
            $this->Cell(60,5,utf8_decode('Fecha Fin:  '.strftime("%A, %d de %B del %Y", strtotime($fechate)).'.-'));
            $this->Ln(5);
            $this->Cell(-1);
            $this->Cell(60,5,utf8_decode('Horario:  '.$horario));
            $this->Ln(5);
            $this->Cell(-1);
            $this->Cell(60,5,utf8_decode('Detalle:  '.$tipo));
            $this->Ln(5);
            $this->Cell(-1);
            $this->Cell(60,5,utf8_decode('Cliente:  '.$cliente));
            $this->Ln(5);
            $this->Cell(-1);
            $this->Cell(60,5,utf8_decode('Dirección:  '.$direccion));
            $this->Ln(5);
            $this->Cell(-1);
            $this->Cell(60,5,utf8_decode('Comuna:  '.$comuna));
            $this->Ln(5);
            $this->Cell(-1);
   ...
   function TablaSimple()
   {
        ...
        $this->Ln(-10);
        $descrcliente = $_REQUEST["descrcliente"];
        $obstec = $_REQUEST["obstec"];
        //otros
        //Título
        ...
        $this->MultiCell(213,5,utf8_decode($descrcliente));
        $this->Ln(10);
        ...
        $this->Ln(-45);
        $this->SetFont('Arial','',11);
        $this->MultiCell(213,5,utf8_decode($obstec));
   }
