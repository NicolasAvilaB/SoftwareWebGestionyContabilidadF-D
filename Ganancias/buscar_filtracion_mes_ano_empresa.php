...
$texto2 = $_POST["texto2"];
$texto3 = $_POST["texto3"];
$consulta = $conexion->query("Call Total_NotaVenta_Ganancia_Filtracion_Mes_Ano_Empr ('$texto','$texto2','$texto3')");
$consulta2 = mysqli_query($conexion2,"Call Total_Factura_Ganancia_Filtracion_Mes_Ano_Empr ('$texto','$texto2','$texto3')");
$consulta3 = mysqli_query($conexion3,"Call Total_NotaCredito_Ganancia_Filtracion_Mes_Ano_Empr ('$texto','$texto2','$texto3')");
$consulta4 = $conexion4->query("Call Total_Gastos_Ganancia_Filtracion_Mes_Ano ('$texto','$texto2')");
$consulta6 = $conexion6->query("Call Total_Retiro_Socios_Ganancia_Filtracion_Mes_Ano ('$texto','$texto2')");
$consulta7 = $conexion7->query("Call Total_Pago_Creditos_Ganancia_Filtracion_Mes_Ano ('$texto','$texto2')");
$consulta9 = $conexion9->query("Call Total_PagoInstalacion_Filtracion_Mes_Ano_Empr ('$texto','$texto2','$texto3')");
$consultax = $conexionx->query("Call Total_Credito_Filtracion_Mes_Ano ('$texto','$texto2')");
$consultaxx = $conexionxx->query("Call Total_Compra_Filtracion_Mes_Ano ('$texto','$texto2')");
$total_notaventa = 0;
$total_factura = 0;
$total_credito = 0;
$total_inst = 0;
...
if ($consulta2) {
    if($row2 =  mysqli_fetch_array($consulta2)){
        ...
        /*$id = $row2[0];
        $conexion02 = new mysqli("127.0.0.1","nicolas_usersoc","usersocifyd","nicolas_socifyd");
        $consulta02 = $conexion02->query("Call Total_Factura_Ganancia_Filtracion_Mes_Ano_Empr0 ('$id')");
        if($row02 = $consulta02->fetch_assoc()){
            $sumaf += intval($row02["Valor"]);
        }
        $consulta02->close();
        $conexion02->more_results();*/
    }
...
if ($consulta3) {
    if($row3 =  mysqli_fetch_array($consulta3)){
        ...
        /*$id = $row3[0];
        $conexion03 = new mysqli("127.0.0.1","nicolas_usersoc","usersocifyd","nicolas_socifyd");
        $consulta03 = $conexion03->query("Call Total_NotaCredito_Ganancia_Filtracion_Mes_Ano_Empr0 ('$id')");
        if($row03 = $consulta03->fetch_assoc()){
            $sumac += intval($row03["Valor"]);
        }
        $consulta03->close();
        $conexion03->more_results();*/
    }
}
if ($consulta4) {
    if($row4 = $consulta4->fetch_assoc()){
        $total_gastos = 0;//$row4["Suma"];
    }
}
if ($consulta6) {
    if($row6 = $consulta6->fetch_assoc()){
        $total_retirosocios = 0;//$row6["Suma"];
    }
}
if ($consulta7) {
    if($row7 = $consulta7->fetch_assoc()){
        $total_pagocreditos = 0;//$row7["Suma"];
    }
}
if ($consulta9) {
...
if ($consultax) {
    if($rowx = $consultax->fetch_assoc()){
        $total_credito2 = 0;//$rowx["Suma"];
    }
}
if ($consultaxx) {
    if($rowxx = $consultaxx->fetch_assoc()){
        $total_compra = 0;//$rowxx["Suma"];
    }
}
...
echo '<br></br>
        ...
        <br></br>';
if($consulta5 = mysqli_query($conexion5,"Call Todo_Gastos_Ganancia_Filtracion_Mes_Ano ('$texto','$texto2')")){
...
if($consulta = mysqli_query($conexion8, "Call Buscar_Factura_Credito_Filtracion_Mes_Ano_Empr ('$texto','$texto2','$texto3')")){
...
if($consulta111 = mysqli_query($conexion111, "Call Mostrar_NotaVenta_Sin_Facturas_Mes_Ano_Empr ('$texto','$texto2','$texto3')")){
