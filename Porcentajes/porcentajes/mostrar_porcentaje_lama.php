...
if($consulta_porc_lama = $conexion->query("Call Mostrar_Porc_Lama")){
    echo "<button id='aplicar_porc'>Guardar</button>
    <label class='titulo_tabla_porcentaje'>Persianas</label>
            <table id='accion5'> 
            <thead>
                <tr>
                    <th>Lama</th>
            ...
            <tbody>";
    if ($consulta_porc_lama) {
        while($row = $consulta_porc_lama->fetch_assoc()){
            $nom_producto = $row["NombreLama"];
    ...
    echo "</tbody></table>";
    $consulta_porc_lama->close();
    $conexion->more_results();
    mysqli_free_result($ejecutar2);
