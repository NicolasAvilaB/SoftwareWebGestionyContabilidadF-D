...
if($consulta_porc_repuestos = $conexion->query("Call Mostrar_Porc_Repuestos")){
    echo "<!--<p>&nbsp;</p>
            <hr class='division'></hr>
            <p>&nbsp;</p>-->
            <button id='aplicar_porc3'>Guardar</button><label class='titulo_tabla_porcentaje'>Repuestos</label>
            <table id='accion5'> 
            <thead>
                <tr>
                    <th>Repuestos</th>
            ...
            <tbody>";
    if ($consulta_porc_repuestos) {
        while($row = $consulta_porc_repuestos->fetch_assoc()){
            $nom_producto = $row["NombreRepuesto"];
