...
$texto = $_POST["texto"];
$texto2 = $_POST["texto2"];
if($consulta = $conexion->query("Call Mostrar_Filtro_Pagos_Admin('$texto','$texto2')")){
    echo "<table id='accion4'>
            <thead>
                <tr>";
                    if (is_numeric($texto2)) {
                        echo "<th><a id='inform' href='generar_informeid_filtrado.php?idpagado=".$texto2."&tecnico=".$texto."' type='button' value='View PDF' class='btn btn-primary' target='_blank'>Informe </a><p></p>Fecha Efectuada</th>";
                    }else{
                        echo "<th>Fecha Efectuada</th>";
                    }
                    echo "<th>Nota Venta</th>
                    <th>Cliente</th>
                    <th>Tipo</th>
                    <th>Cantidad</th>
                    <th>Valor Unit.</th>
        ...
        while($row = $consulta->fetch_assoc()){
          ...
          $bonodescs = $row["Bono_Desc"];
            ...
            echo "<tr>
                ...
                if ($est == '0'){
                    echo "<td></td>  <td contentEditable>".$bonodescs."</td>
                      <td><button id='bono_descuento' data-id_pagos='".$row["Id"]."'>Bono Desc.</button></td>";
                }elseif ($est == '1') {
                  echo "<td><button id='quitar_pagad' class='quitar_pagad' data-id_modificar='".$row["Id"]."' data-fecha_pagado='".$fecha_pag."' data-tipo='".$tip."'>Quitar Pagado</button>  <td contentEditable>".$bonodescs."</td>
                    <td><button id='bono_descuento' data-id_pagos='".$row["Id"]."'>Bono Desc.</button></td></td>";
                }echo "</tr>";
            if ($tip == 'Instalacion' || $tip == 'Rectificación' || $tip == 'Rectificación con Bono Diario'){
                if ($pag == 'Si'){
                    $suma_total += $valor;
