...
$texto3 = $_POST["texto3"];
$texto4 = $_POST["texto4"];
if($consulta = $conexion->query("Call Mostrar_Filtro_Pagos_Admin_Mes_Ano_Empr_Pagados('$texto','$texto2','$texto3','$texto4')")){
        ...
        while($row = $consulta->fetch_assoc()){
          ...
          $bonodescs = $row["Bono_Desc"];
            ...
            echo "<tr>
                ...
                <td><button id='quitar_pagad' class='quitar_pagad' data-id_modificar='".$row["Id"]."' data-fecha_pagado='".$fecha_pag."' data-tipo='".$tip."'>Quitar Pagado</button></td>
                <td contentEditable>".$bonodescs."</td>
                <td><button id='bono_descuento' data-id_pagos='".$row["Id"]."'>Bono Desc.</button></td>
                <td>".$fecha_pag."</td>
                <td>".$row["IDPagado"]."</td>
                <td><button id='ver' class='ver' data-id='".$row["IDPagado"]."'>Ver</button></td></tr>";
            if ($tip == 'Instalacion' || $tip == 'Rectificación' || $tip == 'Rectificación con Bono Diario'){
                if ($pag == 'Si'){
                    $suma_total += $valor;
                }else{}
